...
/*   Updated: 2021/06/05 12:55:25 by ncolin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void)
{
	...
	{
		Intern on_stack;
		Intern on_stack_cpy(on_stack);
	}
	std::cout << std::endl << "On heap" << std::endl;
	{
		Intern *on_heap = new Intern();
		Intern *on_heap_cpy = new Intern(*on_heap);
	...
	try
	{
		Bureaucrat buro("Master",1);
		Intern slave;
		Form* PPF;
		Form* RRF;
		Form* SCF;

		PPF = slave.makeForm("PPF", "target");
		RRF = slave.makeForm("RRF", "target");
		SCF = slave.makeForm("SCF", "target");
